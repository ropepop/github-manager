...
To pause after scrubbed staging is ready so the current Codex chat can write premium public README, docs, and visual drafts:

```bash
python3 -m github_manager run --drop-blocked-files --sanitized-counterparts --public-only --chat-handoff
```

The manager prints a handoff folder, waits for `complete.json`, accepts only `README.md`, `docs/**/*.md`, and `docs/assets/**/*.{png,jpg,jpeg,webp,svg}` drafts, safety-checks them, verifies local image links, and then continues with the staged public copies.
